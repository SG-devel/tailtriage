...
fn queue_stage_and_inflight_are_recorded() {
    ...
    assert!(
        ...
        "native in-flight snapshots should include run-relative timestamps"
    );
...
fn request_stage_and_queue_timing_preserves_order_after_sleep() {
    ...
    let request_started_at_run_us = request
        .started_at_run_us
        .expect("native request should include run-relative start");
    let request_finished_at_run_us = request
        .finished_at_run_us
        .expect("native request should include run-relative finish");
    ...
    let stage_started_at_run_us = stage
        .started_at_run_us
        .expect("native stage should include run-relative start");
    let stage_finished_at_run_us = stage
        .finished_at_run_us
        .expect("native stage should include run-relative finish");
    ...
    let waited_from_run_us = queue
        .waited_from_run_us
        .expect("native queue should include run-relative wait start");
    let waited_until_run_us = queue
        .waited_until_run_us
        .expect("native queue should include run-relative wait finish");
    assert!(waited_until_run_us >= waited_from_run_us);
...
#[test]
fn runtime_snapshot_recorded_through_core_gets_run_relative_timestamp() {
    let tailtriage = build_for_test("payments", "tailtriage-core-runtime-run-relative.json");
...
#[test]
fn older_minimal_run_json_without_run_relative_fields_deserializes_with_none() {
