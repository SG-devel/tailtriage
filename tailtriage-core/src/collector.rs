...
///
/// An admitted unfinished completion dropped while capture is open records one
/// `cancelled` request. For a refused request both fields are inert and record
/// no request or child evidence. Instrumentation alone does not complete the
/// request lifecycle.
...
/// Admission, refusal, completion, and Drop semantics match [`StartedRequest`].
#[must_use = "request completion must be finished explicitly"]
#[derive(Debug)]
...
/// It does not complete the request. A handle returned for a refused request is
/// inert, including for child evidence.
///
/// # Example
///
/// ```no_run
...
/// and helper layers. Like the borrowed handle, it is inert when admission was
/// refused and never owns request completion.
#[derive(Debug, Clone)]
...
/// request, including during panic unwinding; Drop does not panic. After
/// shutdown finalization wins, Drop and explicit finish are inert. A refused
/// request's token is inert from construction.
...
/// capture is open records one `cancelled` request, including during panic
/// unwinding; Drop does not panic. After shutdown finalization wins, Drop and
/// explicit finish are inert. A refused request's token is inert.
#[must_use = "request completion tokens must be finished explicitly"]
#[derive(Debug)]
...
impl Tailtriage {
    ...
    ///
    /// Admission requires open capture and capacity under the effective
    /// [`crate::CaptureLimits::max_requests`] bound, which counts pending plus
    /// retained completed requests. On admission, finish the returned completion
    /// exactly once; on refusal, both returned fields are inert.
    ...
    /// exactly once through the returned completion token. Request options do
    /// not bypass admission. Refusal due to finalization or the effective
    /// request limit yields inert handle and completion fields and no child
    /// evidence; limit refusal updates request truncation accounting.
    ...
    /// handles across task boundaries. Its admission and inert-refusal behavior
    /// is identical to [`Self::begin_request`].
    ...
    ///
    /// Options and admission semantics match [`Self::begin_request_with`]. An
    /// admitted completion owns the finish; a refused completion is inert.
...
impl RequestCompletion<'_> {
    ...
    ///
    /// This consumes the single-owner token. It records one request only when
    /// admission succeeded and capture is still open; a refused or late token
    /// is inert and does not report an error.
