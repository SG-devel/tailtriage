...
///
/// Dropping this token does not auto-finish the request.
...
///
/// Owned variant of [`RequestCompletion`]. Dropping this token does not
...
impl Tailtriage {
    ...
    pub fn begin_request_with(
        ...
        let (request_id, route, kind, pending_key, interval) =
    ...
    fn start_request(
        ...
        (request_id, route, kind, pending_key, interval)
    }
}
...
impl RequestCompletion<'_> {
    ...
    fn finish_internal(&mut self, outcome: Outcome) {
        ...
        let finished = finish_interval(self.interval);
        ...
        self.tailtriage
            .record_request_event(request_event(pending, finished, outcome));
    }
}
...
fn request_event(
    ...
    debug_assert_eq!(
        pending.interval.started_at_unix_ms,
