...
impl Tailtriage {
    ...
    /// Enables or disables capture instrumentation work.
...
impl RequestCompletion<'_> {
    ...
    fn finish_internal(&mut self, outcome: Outcome) {
        ...
        let Some(pending_key) = self.pending_key else {
            self.finished = true;
            return;
        };

        let pending = lock_pending(&self.tailtriage.pending_requests).remove(&pending_key);
...
impl Drop for RequestCompletion<'_> {
    fn drop(&mut self) {
        debug_assert!(
            self.pending_key.is_none() || self.finished || std::thread::panicking(),
