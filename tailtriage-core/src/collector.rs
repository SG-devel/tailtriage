...
pub enum ShutdownError {
    ...
    ///
    /// This is retryable: it occurs before finalization or a sink attempt and
    /// leaves admitted requests open so their completion tokens can resolve.
    UnfinishedRequests {
    ...
    ///
    /// A sink attempt is terminal even on failure; later shutdown calls replay
    /// the failure rather than attempting another write.
    Sink(SinkError),
}
...
///
/// Use `handle` to record queue/stage/inflight evidence, then consume
/// `completion` with an explicit outcome. The completion token exclusively
/// owns request completion; cloned handles do not.
///
...
/// Admission, explicit completion, cancellation-on-Drop, and late-finalization
/// behavior are the same as for [`StartedRequest`].
#[must_use = "request completion must be finished explicitly"]
#[derive(Debug)]
...
///
/// For an admitted request this handle records queue/stage/inflight signals; it
/// never completes the request. When admission is refused because capture is
/// closed or effective request capacity is exhausted, the handle is inert:
/// wrapped futures still run, but no child or in-flight evidence is recorded.
///
/// # Example
///
/// ```no_run
...
/// and helper layers. It has the same admitted-versus-inert behavior and never
/// owns request completion.
#[derive(Debug, Clone)]
...
/// request, including during panic unwinding. Explicit finish consumes and
/// disarms the token, so it cannot add a second completion. A refused request's
/// token is disarmed. After shutdown finalization wins, Drop and explicit
/// finish are inert.
...
///
/// Owned variant of [`RequestCompletion`]. It has the same single-owner
/// completion, cancellation-on-Drop, inert refusal, panic-unwind, and late
...
impl Tailtriage {
    ...
    ///
    /// Admission succeeds only while capture is open and completed-retained
    /// plus pending admitted requests are below the effective
    /// [`crate::CaptureLimits::max_requests`]. On refusal, the returned handle
    /// and completion token are inert; wrapped work still executes but records
    /// no request or child evidence. Refusal at capacity updates truncation.
    /// Finish an admitted completion token exactly once.
    ...
    ///
    /// `options` changes start metadata, not admission or completion semantics.
    /// Queue/stage/inflight instrumentation from an admitted handle records
    /// evidence only; it does not finish the request. Finish must happen
    /// exactly once through the returned completion token. Admission limits,
    /// refusal behavior, and truncation are as described by
    /// [`Self::begin_request`].
    ...
    /// handles across task boundaries. It has the same effective admission
    /// limit, inert-refusal, truncation, and completion behavior as
    /// [`Self::begin_request`].
    ...
    ///
    /// `options` changes start metadata only. Finish the returned completion
    /// token exactly once. Effective admission limits and inert refusal match
    /// [`Self::begin_request`].
    ...
    ///
    /// `snapshot()` is useful before or after shutdown. Before finalization,
    /// `metadata.finalized_at_unix_ms` is `None`; after shutdown it may contain
    /// the finalized timestamp. An integration can set `run_end_reason` when it
    /// closes admissions before core finalization, so absence of the timestamp
    /// does not imply absence of an end reason.
    #[must_use]
    ...
    /// # Errors
    ///
    /// Returns [`ShutdownError::UnfinishedRequests`] for retryable strict
    /// lifecycle failure, or [`ShutdownError::Sink`] for a terminal
    /// serialization/persistence attempt failure. Request-level outcomes are
    /// supplied separately through completion tokens.
...
impl RequestHandle<'_> {
    ...
    ///
    /// Recording queue events does not finish the request. For a refused
    /// request, the returned timer is inert and only awaits the supplied work.
    #[must_use]
    ...
    ///
    /// Recording stage events does not finish the request. For a refused
    /// request, the returned timer is inert and only awaits the supplied work.
    #[must_use]
...
impl RequestCompletion<'_> {
    ...
    ///
    /// This consumes and disarms an admitted token. If the token was refused or
    /// finalization already won, the call is inert.
    ...
    ///
    /// Refused or late completion is inert as described by [`Self::finish`].
    ...
    /// # Errors
    ///
    /// This method does not create new errors. It records `ok` for `Ok` and
    /// `error` for `Err` when the request can still complete, then returns
    /// `result` unchanged, including the original `Err(E)` value.
...
impl OwnedRequestCompletion {
    ...
    ///
    /// This consumes and disarms an admitted token. Refused and late tokens are
    /// inert.
    pub fn finish(mut self, outcome: Outcome) {
        self.finish_internal(outcome);
    }
    ...
    ///
    /// Refused and late tokens are inert.
    pub fn finish_ok(self) {
        self.finish(Outcome::Ok);
    }
    ...
    ///
    /// This method does not create new errors. It records `ok` for `Ok` and
    /// `error` for `Err` when completion remains open, then returns `result`
    /// unchanged, including the original `Err(E)` value.
    pub fn finish_result<T, E>(self, result: Result<T, E>) -> Result<T, E> {
        let outcome = if result.is_ok() {
            Outcome::Ok
        } else {
