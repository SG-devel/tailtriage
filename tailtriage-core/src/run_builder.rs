...
///
/// Push methods validate required event shape and timestamp ordering. Elapsed
/// duration fields such as request/stage `latency_us` and queue `wait_us` are
/// accepted as authoritative completed evidence; [`RunBuilder`] does not
/// synthesize or repair durations from wall-clock timestamps.
///
