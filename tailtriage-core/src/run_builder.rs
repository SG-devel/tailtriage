...
fn duration_us_from_unix_ms_bounds(started_at_unix_ms: u64, finished_at_unix_ms: u64) -> u64 {
    finished_at_unix_ms
...
fn validate_request_event(event: &RequestEvent) -> Result<(), RunBuilderEventError> {
    ...
    if !duration_within_tolerance(event.latency_us, derived_us) {
        return Err(invalid_event(
            ...
            "latency_us",
            format!(
                "observed duration {}us differs from derived duration {}us by more than tolerance {}us",
