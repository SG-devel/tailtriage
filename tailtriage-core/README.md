...
When handles must move across spawned tasks or helper layers, use `begin_request_owned(...)` / `begin_request_with_owned(...)` with `Arc<Tailtriage>`. Owned handles keep the same lifecycle rule: instrumentation does not finish the request, and the completion token must still be finished exactly once.
