...
A `request_id` is the per-run identity of one completed logical request/work item. It must be unique among completed requests in one `Run`; queue and stage events must reuse that ID only for the same logical request. Automatically generated IDs are unique for normal native capture. If you provide an explicit ID from a broader trace/correlation value that can repeat across retries, fanout branches, batch items, or attempts, add attempt/span/branch/item information before passing it to `RequestOptions::request_id(...)`. Duplicate explicit IDs are still recorded so lifecycle completion remains independent, but the run metadata includes a warning because request-scoped attribution can become ambiguous.
