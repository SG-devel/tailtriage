...
fn analyze_run_with_options(run: &Run, options: &AnalyzeOptions) -> Report {
    ...
    let non_default_options = options.non_default_overrides();
    report.analyzer_config = (!non_default_options.is_empty()).then_some(AnalyzerConfigSummary {
        schema_version: 1,
        non_default_options,
    });
    report
...
fn analysis_warnings(run: &Run, suspects: &[Suspect], options: &AnalyzeOptions) -> Vec<String> {
    let mut warnings = evidence::truncation_warnings(run, options);
