...
impl AnalyzeOptions {
    #[must_use]
    pub fn non_default_overrides(&self) -> Vec<crate::AnalyzeConfigOverrideSummary> {
        let default = Self::default();
        let mut overrides = Vec::new();
        macro_rules! push_if_changed {
            ($path:literal, $lhs:expr, $rhs:expr) => {
                if $lhs != $rhs {
                    overrides.push(crate::AnalyzeConfigOverrideSummary {
                        path: $path.to_string(),
                        value: $lhs.to_string(),
                    });
                }
