...
"downstream.min_stage_samples", Usize, "3", "downstream stage eligibility", "Minimum captured samples per stage before downstream dominance is considered.", Some("requires more stage samples"), Some("requires fewer stage samples"), |o| OptionValue::Usize(o.downstream.min_stage_samples), |o, v| { let OptionValue::Usize(v) = v else { unreachable!("registry kind/setter mismatch") }; o.downstream.min_stage_samples = v; };
...
"confidence.ambiguity_score_gap", U8, "4", "ambiguity warning", "Maximum score gap between top suspects to emit ambiguity warning.", Some("allows wider near-tie gaps"), Some("requires tighter near-tie gaps"), |o| OptionValue::U8(o.confidence.ambiguity_score_gap), |o, v| { let OptionValue::U8(v) = v else { unreachable!("registry kind/setter mismatch") }; o.confidence.ambiguity_score_gap = v; };
"evidence.low_completed_request_threshold", Usize, "20", "evidence quality warnings", "Completed-request threshold below which low-sample warnings and conservative confidence limits apply.", Some("requires more completed requests to avoid low-sample warnings"), Some("requires fewer completed requests to avoid low-sample warnings"), |o| OptionValue::Usize(o.evidence.low_completed_request_threshold), |o, v| { let OptionValue::Usize(v) = v else { unreachable!("registry kind/setter mismatch") }; o.evidence.low_completed_request_threshold = v; };
"route.min_request_count", Usize, "3", "route breakdown eligibility", "Minimum per-route completed request count required for route breakdown inclusion.", Some("filters out more low-volume routes"), Some("includes more low-volume routes"), |o| OptionValue::Usize(o.route.min_request_count), |o, v| { let OptionValue::Usize(v) = v else { unreachable!("registry kind/setter mismatch") }; o.route.min_request_count = v; };
"route.breakdown_limit", Usize, "10", "route breakdown output size", "Maximum number of route breakdown entries emitted in one report.", Some("allows more route entries"), Some("allows fewer route entries"), |o| OptionValue::Usize(o.route.breakdown_limit), |o, v| { let OptionValue::Usize(v) = v else { unreachable!("registry kind/setter mismatch") }; o.route.breakdown_limit = v; };
"route.emit_on_divergent_suspects", Bool, "true", "route divergence warning", "Whether to emit a global warning when route-level primary suspects diverge.", None, None, |o| OptionValue::Bool(o.route.emit_on_divergent_suspects), |o, v| { let OptionValue::Bool(v) = v else { unreachable!("registry kind/setter mismatch") }; o.route.emit_on_divergent_suspects = v; };
"route.slowest_to_fastest_p95_ratio_numerator", U64, "3", "route divergence detection", "Numerator for the slowest-to-fastest route p95 ratio threshold.", Some("requires larger slowest/fastest disparity"), Some("requires smaller slowest/fastest disparity"), |o| OptionValue::U64(o.route.slowest_to_fastest_p95_ratio_numerator), |o, v| { let OptionValue::U64(v) = v else { unreachable!("registry kind/setter mismatch") }; o.route.slowest_to_fastest_p95_ratio_numerator = v; };
"route.slowest_to_fastest_p95_ratio_denominator", U64, "2", "route divergence detection", "Denominator for the slowest-to-fastest route p95 ratio threshold.", Some("requires smaller slowest/fastest disparity"), Some("requires larger slowest/fastest disparity"), |o| OptionValue::U64(o.route.slowest_to_fastest_p95_ratio_denominator), |o, v| { let OptionValue::U64(v) = v else { unreachable!("registry kind/setter mismatch") }; o.route.slowest_to_fastest_p95_ratio_denominator = v; };
"route.slowest_to_global_p95_ratio_numerator", U64, "5", "route divergence detection", "Numerator for the slowest-route to global p95 ratio threshold.", Some("requires larger slowest/global disparity"), Some("requires smaller slowest/global disparity"), |o| OptionValue::U64(o.route.slowest_to_global_p95_ratio_numerator), |o, v| { let OptionValue::U64(v) = v else { unreachable!("registry kind/setter mismatch") }; o.route.slowest_to_global_p95_ratio_numerator = v; };
"route.slowest_to_global_p95_ratio_denominator", U64, "4", "route divergence detection", "Denominator for the slowest-route to global p95 ratio threshold.", Some("requires smaller slowest/global disparity"), Some("requires larger slowest/global disparity"), |o| OptionValue::U64(o.route.slowest_to_global_p95_ratio_denominator), |o, v| { let OptionValue::U64(v) = v else { unreachable!("registry kind/setter mismatch") }; o.route.slowest_to_global_p95_ratio_denominator = v; };
"temporal.min_request_count", Usize, "20", "temporal segmentation eligibility", "Minimum completed requests required before temporal early/late segmentation is considered.", Some("requires more requests before temporal analysis"), Some("requires fewer requests before temporal analysis"), |o| OptionValue::Usize(o.temporal.min_request_count), |o, v| { let OptionValue::Usize(v) = v else { unreachable!("registry kind/setter mismatch") }; o.temporal.min_request_count = v; };
"temporal.min_segment_request_count", Usize, "8", "temporal segmentation eligibility", "Minimum requests required in each temporal segment.", Some("requires larger per-segment sample size"), Some("allows smaller per-segment sample size"), |o| OptionValue::Usize(o.temporal.min_segment_request_count), |o, v| { let OptionValue::Usize(v) = v else { unreachable!("registry kind/setter mismatch") }; o.temporal.min_segment_request_count = v; };
"temporal.share_shift_permille", U64, "200", "temporal shift detection", "Minimum queue/service share shift (permille) to flag temporal movement.", Some("requires larger share movement"), Some("allows smaller share movement"), |o| OptionValue::U64(o.temporal.share_shift_permille), |o, v| { let OptionValue::U64(v) = v else { unreachable!("registry kind/setter mismatch") }; o.temporal.share_shift_permille = v; };
"temporal.p95_shift_ratio_numerator", U64, "3", "temporal shift detection", "Numerator for temporal p95 ratio shift threshold.", Some("requires larger p95 movement"), Some("requires smaller p95 movement"), |o| OptionValue::U64(o.temporal.p95_shift_ratio_numerator), |o, v| { let OptionValue::U64(v) = v else { unreachable!("registry kind/setter mismatch") }; o.temporal.p95_shift_ratio_numerator = v; };
"temporal.p95_shift_ratio_denominator", U64, "2", "temporal shift detection", "Denominator for temporal p95 ratio shift threshold.", Some("requires smaller p95 movement"), Some("requires larger p95 movement"), |o| OptionValue::U64(o.temporal.p95_shift_ratio_denominator), |o, v| { let OptionValue::U64(v) = v else { unreachable!("registry kind/setter mismatch") }; o.temporal.p95_shift_ratio_denominator = v; };
"temporal.emit_on_suspect_shift", Bool, "true", "temporal suspect-shift warning", "Whether temporal suspect-shift warnings are emitted when shifts are detected.", None, None, |o| OptionValue::Bool(o.temporal.emit_on_suspect_shift), |o, v| { let OptionValue::Bool(v) = v else { unreachable!("registry kind/setter mismatch") }; o.temporal.emit_on_suspect_shift = v; };
"temporal.suppress_runtime_sparse_suspect_shift_without_supporting_movement", Bool, "true", "temporal warning suppression", "Whether to suppress runtime-sparse temporal suspect-shift warnings when supporting movement is absent.", None, None, |o| OptionValue::Bool(o.temporal.suppress_runtime_sparse_suspect_shift_without_supporting_movement), |o, v| { let OptionValue::Bool(v) = v else { unreachable!("registry kind/setter mismatch") }; o.temporal.suppress_runtime_sparse_suspect_shift_without_supporting_movement = v; };
}
...
mod tests {
    ...
    fn every_entry_reapplies_its_own_typed_value_without_change() {
        for entry in OPTION_ENTRIES {
            let original = AnalyzeOptions::default();
            let mut reapplied = original.clone();
            entry.apply(&mut reapplied, entry.read(&original));
            assert_eq!(reapplied, original, "{}", entry.path);
        }
    }
}
