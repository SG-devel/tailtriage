...
- Native/tracing scenario evidence parity checks (release CI): `validate-tracing-parity all` provides tracing contract parity checks for scenario coverage, expected evidence presence, route coverage, capture-mode behavior across light and investigation, and runtime/non-runtime boundaries. These checks do not require byte-for-byte artifact equality or exact suspect-ranking equality in every scenario. The queue tiny-limit retention parity gate (`validate-tracing-retention-parity`) runs in both `light` and `investigation` modes and checks retained counts, dropped counters, `truncation.limits_hit`, and `metadata.effective_core_config`.


Runtime-sensitive tracing parity uses deterministic manually injected runtime snapshots and must not rely on ambient background sampler noise or sampler metadata. When tracing parity scenarios need deterministic/manual runtime-pressure evidence, they call `disable_background_sampler()` in `TracingTokioSession` and inject snapshots with `record_runtime_snapshot(...)`. Runtime-sensitive parity requires non-empty runtime snapshots, the scenario-specific runtime field evidence, and the explicit disabled-background-sampler lifecycle warning. Output remains a Run artifact for later `tailtriage analyze`, and findings are evidence-ranked suspects, not proof.
