...
- Broad tracing parity checks (release CI): scenario coverage, expected evidence presence, route coverage, mode parity, and runtime/non-runtime semantics (`validate-tracing-parity`). The queue tiny-limit retention parity gate checks retained counts, dropped counters, `truncation.limits_hit`, and `metadata.effective_core_config` (`validate-tracing-retention-parity`).
