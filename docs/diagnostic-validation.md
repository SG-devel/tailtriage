...
- Native/tracing parity checks (release CI): `validate-tracing-parity all` provides broad tracing parity checks for scenario coverage, expected evidence presence, route coverage, mode parity, and runtime/non-runtime semantics. The queue tiny-limit retention parity gate (`validate-tracing-retention-parity`) checks retained counts, dropped counters, `truncation.limits_hit`, and `metadata.effective_core_config`.


Tracing validation supports deterministic runtime-sensitive scenarios by disabling `TracingTokioSession` background sampling and injecting manual runtime snapshots. Validation assertions treat runtime-pressure evidence as triage input (evidence-ranked suspects and next checks), not root-cause proof, and keep analysis as a separate `tailtriage analyze <run.json>` step when direct Run JSON persistence is used.
