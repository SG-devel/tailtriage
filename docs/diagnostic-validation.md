...
- Native/tracing parity checks (release CI): `validate-tracing-parity all` provides broad tracing parity checks for scenario coverage, expected evidence presence, route coverage, mode parity across both `light` and `investigation` capture modes, and runtime/non-runtime semantics. The queue tiny-limit retention parity gate (`validate-tracing-retention-parity`) runs in both capture modes and checks retained counts, dropped counters, `truncation.limits_hit`, and `metadata.effective_core_config`.


When tracing parity scenarios need deterministic/manual runtime-pressure evidence, they disable the background sampler and inject snapshots with `record_runtime_snapshot(...)`. Runtime-sensitive parity requires runtime snapshots plus either effective Tokio sampler metadata or the explicit disabled-background-sampler lifecycle warning. Parity validates request/stage/queue/runtime-evidence shape and diagnosis direction, not exact latency equality. Output remains a Run artifact for later `tailtriage analyze`, and findings are evidence-ranked suspects, not proof.
