...
- Native/tracing parity checks (release CI): `validate-tracing-parity all` runs every parity scenario in both `light` and `investigation` capture modes and checks scenario coverage, expected evidence presence, route coverage, mode parity, and runtime/non-runtime semantics. The queue tiny-limit retention parity gate (`validate-tracing-retention-parity`) also runs in both capture modes and checks retained counts, dropped counters, `truncation.limits_hit`, and `metadata.effective_core_config`.


When tracing parity scenarios need deterministic/manual runtime-pressure evidence, they may call `disable_background_sampler()` in `TracingTokioSession` and inject snapshots with `record_runtime_snapshot(...)`. Runtime-sensitive parity requires deterministic manually injected runtime snapshots plus either effective Tokio sampler metadata or the explicit disabled-background-sampler lifecycle warning. Output remains a Run artifact for later `tailtriage analyze`; parity means evidence shape and diagnosis direction rather than exact latency equality, and findings are evidence-ranked suspects, not production root-cause proof.
