...
- Native/tracing parity checks (release CI): `validate-tracing-parity all` provides broad tracing parity checks for scenario coverage, expected evidence presence, route coverage, capture-mode parity across light and investigation, and runtime/non-runtime semantics. The queue tiny-limit retention parity gate (`validate-tracing-retention-parity`) runs in both `light` and `investigation` modes and checks retained counts, dropped counters, `truncation.limits_hit`, and `metadata.effective_core_config`.


Runtime-sensitive tracing parity uses deterministic manually injected runtime snapshots and must not rely on ambient background sampler noise. When tracing parity scenarios need deterministic/manual runtime-pressure evidence, they may call `disable_background_sampler()` in `TracingTokioSession` and inject snapshots with `record_runtime_snapshot(...)`. Runtime-sensitive parity requires runtime snapshots plus the explicit disabled-background-sampler lifecycle warning. Ambient sampler metadata/noise does not satisfy this deterministic contract. Runtime snapshots still must carry scenario-relevant runtime field evidence (`blocking_queue_depth` for blocking pressure, `global_queue_depth`/`local_queue_depth` for executor pressure). Output remains a Run artifact for later `tailtriage analyze`, and findings are evidence-ranked suspects, not proof.
