...
Runtime-sensitive tracing parity uses deterministic manually injected runtime snapshots and must not rely on ambient background sampler noise. When tracing parity scenarios need deterministic/manual runtime-pressure evidence, they call `disable_background_sampler()` in `TracingTokioSession` and inject snapshots with `record_runtime_snapshot(...)`. The CI parity contract requires the disabled-background-sampler lifecycle warning for runtime-sensitive tracing artifacts, plus the relevant runtime field evidence (`blocking_queue_depth` for blocking, and `global_queue_depth`/`local_queue_depth` for executor). Output remains a Run artifact for later `tailtriage analyze`, and findings are evidence-ranked suspects, not proof of production root cause.
