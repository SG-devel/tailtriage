...
When tracing parity scenarios need deterministic runtime-pressure evidence, they may disable background runtime sampling in `TracingTokioSession` and inject manual snapshots. Output remains a Run artifact for later `tailtriage analyze`, and findings are evidence-ranked suspects, not proof.
