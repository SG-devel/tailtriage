...
CI runs one bounded runtime-cost smoke gate on the Ubuntu extended release leg:
...
Runtime-cost CI is a bounded smoke gate with broad tracing/native sanity thresholds. It is not a rigorous benchmark suite and should not be interpreted as stable performance characterization. The gate enforces tracing/native hard checks (p95 <= 1.10x native and throughput >= 0.90x native), a 2% soft warning band (p95 > 1.02x or throughput < 0.98x), and required tracing evidence shape. CI validates runtime-cost output in-place and does not upload runtime-cost artifacts by default. CI logs print compact runtime-cost tables by default, while full JSON remains in artifacts (`runtime-cost-summary.json`) and can be printed locally with `--print-json`. Full runtime-cost measurement remains a local/developer-run path via the canonical command above. Results remain machine/workload/profile scoped.
...
Numbers are directional and machine/workload/profile scoped; this bounded smoke gate catches meaningful regressions with broad sanity thresholds, not rigorous benchmark conclusions.
