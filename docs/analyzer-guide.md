...
| --- | --- | --- | --- | --- |
| `application_queue_pressure` | Requests spent a material part of their observed time waiting in an application queue. | Queue-time share, queue depth, sample coverage, and whether durations are complete or lower bounds. | Inspect admission limits and producer bursts at the named queue; then compare queue wait after one controlled capacity or concurrency change. | That the queue implementation is proven faulty, or that adding workers must fix it. |
