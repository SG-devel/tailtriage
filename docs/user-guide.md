...
Create a `Tailtriage` instance with `Tailtriage::builder(...)`, start one reusable request context per work item with `begin_request(...)`, wrap explicit queue/stage boundaries, then call `shutdown()` at lifecycle end.
