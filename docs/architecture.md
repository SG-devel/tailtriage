...
It converts tracing evidence into standard `tailtriage_core::Run` artifacts, including stable completed-span JSONL as an intermediate input format (`tailtriage.tracing-span.v1`), and does not introduce a tracing-specific analyzer path. It does not implement OpenTelemetry/OTLP.
