...
`tailtriage-tracing/examples/tracing_spans.jsonl` is a deterministic, manually authored
example/intake fixture containing one completed request span, queue span, and stage span in the
stable wrapper format. It is sourced from the example-facing tracing import shape rather than from
a generator; current repository code and documentation provide no supported generation or refresh
command. Its authoritative consumer is `tailtriage-tracing/tests/tracing_examples.rs`, which loads
the exact committed path to prove completed-span JSONL import shape, path-versus-reader import
parity, analyzability of the imported Run, and the absence of fabricated runtime snapshots. Change
its bytes only by direct, manually reviewed edits alongside that consumer when the example-facing
import contract intentionally changes. Its fixed timestamps, IDs, fields, and source order make
the committed input deterministic; importer warnings and resulting assertions remain the proof,
not a generated byte-for-byte output. Keep this fixture in place with explicit lineage ownership.
...
no generator and is not an equivalence scenario; keep it distinct. Although all three families use
tracing JSONL, the example fixture protects stable wrapper imports through example-facing test
paths, this test fixture protects parser/CLI boundaries, and `tests/fixtures/equivalence/*.jsonl`
provides completed-span inputs for native-versus-tracing comparison against independent Run and
Report oracles.
...
| `report_contract` / `analysis_report` | The 18 canonical before/after (or baseline/mitigated) manifest paths in `demos/{queue_service,blocking_service,executor_pressure_service,downstream_service,mixed_contention_service,cold_start_burst_service,db_pool_saturation_service,shared_state_lock_service,retry_storm_service}/fixtures/` | Benchmark inspects already report-shaped teaching outputs; analyzer/importer does not execute; accuracy ineligible | Owned/generated by the demo boundary below, not the diagnostic lock; keep linked use |
...
| --- | --- | --- |
| `queue_service` | `before-analysis.json`, `after-analysis.json` | Drift checker and diagnostic report contracts; the canonical before report also owns CLI smoke and docs-contract checks |
| `blocking_service` | before and after | Drift checker and diagnostic report contracts; before also owns CLI smoke |
| `executor_pressure_service` | before and after | Drift checker and diagnostic report contracts; before also owns CLI smoke |
| `downstream_service` | before and after | Same boundaries plus the drift-owned comparison; before also owns CLI smoke |
...
may rewrite exactly the 18 analysis files described above and
`demos/downstream_service/fixtures/before-after-comparison.json`—19 paths total. Check and refresh
use that same map. It does **not** own every file in a fixture directory.

The former four derived sample aliases are not committed. CLI smoke and documentation checks
consume the canonical before reports directly, and the duplicate report-contract manifest cases
were removed. This leaves the 11 analyzer-executed accuracy observations unchanged.

`demos/downstream_service/fixtures/before-after-comparison.json` is the only committed comparison.
It is created by the private shared runner, checked and refreshed by the drift owner, and used by
the downstream teaching guide. Other comparison output is generated only in temporary or `target/`
directories by workflows that consume it; it is not a committed fixture contract.
...
| Collector limits | `python3 scripts/measure_collector_limits.py --artifact-dir <dir>` directly produces raw/summary files for the selected matrix. | `validation/collector-limits/latest/scorecard.md` is a stable checked-in availability/status note. Manual-reviewed only; keep. Evidence is machine/workload/profile sensitive and proves visible bounded drops, warnings, and downgrade behavior—not no drops. |
...
- Canonical before Reports own teaching, smoke, and linked diagnostic report-contract use directly;
  no derived sample aliases are retained.
