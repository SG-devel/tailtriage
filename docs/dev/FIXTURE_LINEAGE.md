...
| --- | --- | --- | --- |
| `analyzer_execution` / `run_artifact` | `corpus/run-artifacts/{low-request-insufficient,no-queue-events,no-runtime-snapshots,no-stage-events,truncated-queues}.json` and `corpus/{partial-evidence-completed-only,partial-evidence-mixed,partial-evidence-queue-only,partial-evidence-stage-only,cancelled-request-with-partial-child}.json` | `scripts/diagnostic_benchmark.py` loads through real CLI Run intake and analyzer; the manifest-classified observations are accuracy eligible. `tailtriage-analyzer/src/tests.rs` also deserializes the direct-corpus partial/cancelled Runs to protect defaults and partial flags. | Manually authored deterministic inputs; integrity-lock protected; keep |
...
may rewrite the analysis files described above and
`demos/downstream_service/fixtures/before-after-comparison.json`. Check and refresh
