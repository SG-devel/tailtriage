...
| In-process analysis | Typed `tailtriage_core::Run` → `tailtriage_analyzer::analyze_run` → typed `Report` → optional canonical renderers | [Analyzer package](../../tailtriage-analyzer/README.md) |
...
| Explicit permissive/ambiguous saved artifacts | Analyzer analysis remains permissive by default; generic strict `Run` validation is explicitly composed through `tailtriage_core::validate_run_strict`. The CLI owns `--allow-ambiguous-artifact` and saved-artifact acceptance policy. See the [CLI policy](../../tailtriage-cli/README.md) and [analyzer report contract](../../tailtriage-analyzer/README.md). |
...
| Tracing-equivalence artifacts | Native `Run`, stable completed-span JSONL, and expected equivalence evidence are owned by `tailtriage-tracing` tests and [`tests/fixtures/equivalence/`](../../tailtriage-tracing/tests/fixtures/equivalence). | [Tracing README](../../tailtriage-tracing/README.md) and [validation equivalence tracks](VALIDATION.md#nativetracing-equivalence-tracks) |
