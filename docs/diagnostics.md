...
- `temporal_segments`: always present and usually empty. It is populated only when conservative within-run early/late checks detect material signal movement (for example suspect-kind shifts or large p95 movement). Temporal segments are supporting within-run hints only; global `primary_suspect` remains the full-run triage lead. Temporal segments do not prove phase-specific root cause. A top-level warning reports when early/late p95 latency shifts materially. Runtime and in-flight phase attribution is timestamp-filtered to each segment window and is limited when segment-filtered samples are sparse; overlapping early/late windows under concurrent requests are explicitly warned as approximate attribution.
