...
- `temporal_segments`: always present and usually empty. It is populated only when conservative within-run early/late checks detect material signal movement (for example suspect-kind shifts or large p95 movement). Temporal segments are supporting within-run hints only; global `primary_suspect` remains the full-run triage lead. Temporal segments do not prove phase-specific root cause. A large early/late p95 change adds the warning `Temporal segments show a large p95 latency shift between early and late requests.` Runtime and in-flight phase attribution is timestamp-filtered to each segment window, is limited when segment-filtered samples are sparse, and is approximate when early/late segment windows overlap under concurrent requests.
