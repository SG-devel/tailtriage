...
- Configure `max_open_spans` to bound in-flight span tracking; completed request/stage/queue retention is bounded by resolved core capture limits.
- Completed-span JSONL streaming happens before in-memory Run retention, so the file can preserve valid completed candidate spans beyond retained Run evidence limits.
