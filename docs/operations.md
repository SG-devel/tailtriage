...
- `completed_span_jsonl_path(...)` writes retained, semantically valid completed-span JSONL on shutdown.
- Configure `max_open_spans`` to keep memory bounded.
- `completed_span_jsonl_path(...)` output can be replayed into the same retained request/stage/queue evidence when imported with matching service metadata.
- Completed-span JSONL is retained `tailtriage` evidence export, not a generic tracing log stream and not OTel/OTLP.
