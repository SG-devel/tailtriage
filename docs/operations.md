...
- `completed_span_jsonl_path(...)` writes retained, semantically valid completed-span JSONL at shutdown.
- Configure `max_open_spans`` to keep memory bounded.
- With matching service metadata, retained completed-span JSONL replays into the same retained request/stage/queue evidence during import.
- Completed-span JSONL is retained request/stage/queue evidence export, not a generic tracing log stream and not OTel/OTLP.
