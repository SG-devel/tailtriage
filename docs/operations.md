...
- Configure `max_open_spans` and core capture limits to keep memory bounded.
- Completed-span JSONL streaming happens before in-memory `core capture limits` retention, so the file can preserve spans beyond in-memory retained completed spans.
