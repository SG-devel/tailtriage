...
`TracingTokioSession` uses the same shared capture-mode and capture-limit model as native capture. Configure retention through `mode(...)`, `capture_limits(...)`, or `capture_limits_override(...)` rather than a tracing-specific runtime knob. Tracing-only runs still do not fabricate runtime snapshots, so runtime-pressure evidence requires Tokio session/runtime sampler coupling.
