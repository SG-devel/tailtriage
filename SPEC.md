...
Accepted in-flight transitions remain in the bounded Run artifact, including the final zero when retention permits. While capture is open, the live collector map retains only positive gauge counts: after boundedly attempting the zero snapshot, it removes the zero-valued label in the same state transition. Artifact retention and live-state cleanup are independent, so saturation may drop the snapshot while cleanup still occurs. An in-flight guard dropped after finalization is inert.
