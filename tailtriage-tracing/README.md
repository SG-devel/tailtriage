...
`TracingTokioSession` uses the same core capture-limit model for runtime snapshot retention (`mode(...)`, `capture_limits(...)`, `capture_limits_override(...)`); there is no tracing-specific `max_runtime_snapshots(...)` builder knob.
