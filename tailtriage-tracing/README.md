...
For `TracingTokioSession`, runtime snapshot retention also uses the same core capture-limit model. Its run metadata time bounds cover merged retained tracing evidence plus retained runtime snapshots for triage context, not root-cause proof:
