...
Tracing intake import and native capture share the same CaptureMode/CaptureLimits semantics for request/stage/queue evidence retention. Offline tracing JSONL import does not fabricate runtime snapshots. Runtime-pressure evidence still requires runtime snapshots/Tokio sampler coupling. Runtime-sensitive parity uses deterministic/manual runtime snapshots and requires the explicit disabled-background-sampler lifecycle warning (deterministic/manual mode via `disable_background_sampler()` + `record_runtime_snapshot(...)`) plus the relevant runtime field evidence for the scenario under test. This supports repeatable triage evidence and does not prove production root cause.
