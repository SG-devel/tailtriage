...
- Newer live tracing output includes run-relative monotonic offsets for request, stage, and queue spans so the analyzer can group evidence in run time more accurately.
- Imported JSONL may omit those run-relative offsets; when they are absent, the analyzer falls back to Unix-ms wall-clock timestamps for temporal grouping.
