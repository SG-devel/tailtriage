...
Tracing intake import and native capture share the same CaptureMode/CaptureLimits semantics for request/stage/queue evidence retention. Offline tracing JSONL import does not fabricate runtime snapshots. Runtime-pressure evidence still requires runtime snapshots/Tokio sampler coupling. Runtime-sensitive parity should include runtime snapshots plus the explicit disabled-background-sampler lifecycle warning (deterministic/manual mode via `disable_background_sampler()` + `record_runtime_snapshot(...)`); ambient sampler metadata/noise is not accepted as a substitute. Runtime field evidence still must be present in snapshots for scenario-specific checks (for example `blocking_queue_depth` for blocking pressure and `global_queue_depth`/`local_queue_depth` for executor pressure).
