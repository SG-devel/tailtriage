...
fn parse_record(
    ...
    if let Some(obj) = value.as_object() {
        if let Some(format) = obj.get("format") {
            let Some(format_marker) = format.as_str() else {
                ...
                if strict || matches!(mode, JsonlParseMode::TailtriageWrapperOnly) {
    ...
    if matches!(mode, JsonlParseMode::TailtriageWrapperOnly) {
        ...
        return Err(ImportError::ExpectedTailtriageWrapper { reason });
    }
    ...
    if let Some(obj) = value.as_object() {
        let looks_like_tracing_log_envelope = obj.contains_key("timestamp")
            || obj.contains_key("level")
            || obj.contains_key("target");
        if looks_like_tracing_log_envelope {
            if has_tt {
                let message = format!(
                    "line {line_no}: tracing-log envelope fields (timestamp/level/target) are unsupported import input; compatible mode only accepts direct SpanRecord JSON or normalized completed-span shape"
                );
                if strict {
                    return Err(ImportError::StrictViolation(message));
                }
    ...
    if has_tt {
        let message = format!(
            "line {line_no}: tailtriage JSONL record must use normalized span shape with explicit timestamps"
        );
        if strict {
            Err(ImportError::StrictViolation(message))
        } else {
...
"#;
    ...
    #[test]
    fn compatible_mode_rejects_close_event_like_fmt_envelopes_even_with_timing() {
...
{"event":"close","span":{"name":"st","fields":{"tt.kind":"stage","tt.request_id":"r1","tt.stage":"db"}},"started_at_unix_ms":5,"finished_at_unix_ms":8}"#;
    ...
    #[test]
    fn compatible_mode_rejects_close_event_like_nested_timestamps() {
        let input = r#"{"span":{"name":"req","started_at_unix_ms":1,"finished_at_unix_ms":10,"fields":{"tt.kind":"request","tt.request_id":"r1","tt.route":"/a"}}}
...
{"span":{"name":"q","started_at_unix_ms":1,"finished_at_unix_ms":2},"tt.kind":"queue","tt.request_id":"r1","tt.queue":"permits","tt.depth_at_start":7}"#;
    ...
    #[test]
    fn default_import_rejects_non_wrapper_record() {
    ...
    #[test]
    fn wrapper_only_unsupported_marker_errors_in_all_strictness_modes() {
    ...
    #[test]
    fn wrapper_only_mode_non_wrapper_tt_json_returns_expected_wrapper_error() {
        let input = r#"{"tt.kind":"request","tt.request_id":"r1","tt.route":"/checkout"}"#;
        let err = import_jsonl_reader_with_mode(
            Cursor::new(input),
            ImportOptions::new("svc"),
    ...
    #[test]
    fn import_jsonl_reader_unwrapped_non_strict_errors_by_default() {
        let unwrapped = r#"{"span":{"name":"req","started_at_unix_ms":1,"finished_at_unix_ms":2,"fields":{"tt.kind":"request","tt.request_id":"r1","tt.route":"/a"}}}"#;
