...
{"span":{"name":"q","started_at_unix_ms":1,"finished_at_unix_ms":2},"tt.kind":"queue","tt.request_id":"r1","tt.queue":"permits","tt.depth_at_start":7}"#;
    ...
    #[test]
    fn wrapper_v1_preserves_run_relative_offsets() {
        let input = r#"
{"format":"tailtriage.tracing-span.v1","span":{"name":"req","started_at_unix_ms":10,"started_at_run_us":1000,"finished_at_unix_ms":20,"finished_at_run_us":2000,"duration_us":1234,"fields":{"tt.kind":"request","tt.request_id":"r1","tt.route":"/a"}}}
{"format":"tailtriage.tracing-span.v1","span":{"name":"st","started_at_unix_ms":11,"started_at_run_us":1100,"finished_at_unix_ms":18,"finished_at_run_us":1800,"duration_us":1234,"fields":{"tt.kind":"stage","tt.request_id":"r1","tt.stage":"db"}}}
{"format":"tailtriage.tracing-span.v1","span":{"name":"q","started_at_unix_ms":12,"started_at_run_us":1200,"finished_at_unix_ms":17,"finished_at_run_us":1700,"duration_us":1234,"fields":{"tt.kind":"queue","tt.request_id":"r1","tt.queue":"permits"}}}
"#;
        let imported = import_jsonl_reader_with_mode(
            Cursor::new(input),
            ImportOptions::new("svc"),
        ...
        .unwrap();
        let run = imported.run();
        assert_eq!(run.requests[0].started_at_run_us, Some(1_000));
        assert_eq!(run.requests[0].finished_at_run_us, Some(2_000));
        assert_eq!(run.stages[0].started_at_run_us, Some(1_100));
        assert_eq!(run.stages[0].finished_at_run_us, Some(1_800));
        assert_eq!(run.queues[0].waited_from_run_us, Some(1_200));
        assert_eq!(run.queues[0].waited_until_run_us, Some(1_700));
    }

    #[test]
    fn wrapper_v1_without_run_relative_offsets_imports_none() {
        let input = r#"{"format":"tailtriage.tracing-span.v1","span":{"name":"req","started_at_unix_ms":1,"finished_at_unix_ms":2,"fields":{"tt.kind":"request","tt.request_id":"r1","tt.route":"/a"}}}"#;
        let imported = import_jsonl_reader_with_mode(
            Cursor::new(input),
            ImportOptions::new("svc"),
            JsonlParseMode::TailtriageWrapperOnly,
    ...
    #[test]
    fn wrapper_v1_non_strict_duration_mismatch_keeps_duration_authoritative() {
        ...
        .unwrap();
        assert_eq!(imported.run().requests[0].latency_us, 1_234);
        assert_eq!(imported.run().requests[0].started_at_run_us, Some(100));
        assert_eq!(imported.run().requests[0].finished_at_run_us, Some(200));
        assert!(imported
            .warnings()
            .iter()
