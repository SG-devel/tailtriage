...
mod tests {
    ...
    #[test]
    fn live_recorder_request_conversion_populates_run_relative_timing() {
        with_recorder(|recorder| {
            let span = tracing::info_span!(
                "request",
                tt.kind = "request",
    ...
    #[test]
    fn live_recorder_stage_conversion_populates_run_relative_timing() {
        with_recorder(|recorder| {
    ...
    #[test]
    fn live_recorder_queue_conversion_populates_run_relative_timing() {
        with_recorder(|recorder| {
            let request = tracing::info_span!(
                "request",
                tt.kind = "request",
    ...
    #[test]
    fn retained_request_stage_queue_preserve_run_relative_timing() {
        ...
        run.stages.push(tailtriage_core::StageEvent {
            ...
            stage: "db".into(),
            started_at_unix_ms: 100,
            started_at_run_us: Some(1_100),
            finished_at_unix_ms: 101,
            finished_at_run_us: Some(1_900),
            latency_us: 1_000,
            success: true,
        });
        run.queues.push(tailtriage_core::QueueEvent {
            ...
            waited_from_run_us: Some(1_200),
            waited_until_unix_ms: 101,
            waited_until_run_us: Some(1_300),
            wait_us: 1_000,
            depth_at_start: None,
        });

        let spans = retained_span_records_from_run(&run);
        assert_eq!(spans[0].started_at_run_us_ref(), Some(1_000));
        assert_eq!(spans[0].finished_at_run_us_ref(), Some(2_000));
        assert_eq!(spans[1].started_at_run_us_ref(), Some(1_100));
        assert_eq!(spans[1].finished_at_run_us_ref(), Some(1_900));
        assert_eq!(spans[2].started_at_run_us_ref(), Some(1_200));
        assert_eq!(spans[2].finished_at_run_us_ref(), Some(1_300));
    }
