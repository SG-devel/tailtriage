...
class VerificationError(RuntimeError):
    """Recorded sensitivity evidence does not match the current generator."""
...
def plan_bytes(experiments: list[dict[str, Any]]) -> bytes:
    rows = [{**experiment, "overrides": ";".join(experiment["overrides"])}
            for experiment in experiments]
    return csv_bytes(rows, ["experiment_id", "family", "variant", "input_id", "overrides"])


def verify_recorded_plan(output: Path, experiments: list[dict[str, Any]],
                         inputs: dict[str, dict[str, Any]]) -> None:
    """Verify that current plan and input generation exactly reproduce recorded bytes."""
    plan_path = output / "experiment-plan.csv"
    if not plan_path.is_file():
        raise VerificationError(f"missing recorded experiment plan: {plan_path}")
    if plan_path.read_bytes() != plan_bytes(experiments):
        raise VerificationError("recorded experiment plan differs from the current generator")

    inputs_dir = output / "inputs"
    expected = {f"{input_id}.json" for input_id in inputs}
    recorded = {path.name for path in inputs_dir.glob("*.json")} if inputs_dir.is_dir() else set()
    ...
    if missing or extra:
        ...
        raise VerificationError("recorded input file set differs: " + "; ".join(details))
    for input_id, run in sorted(inputs.items()):
        path = inputs_dir / f"{input_id}.json"
        expected_bytes = canonical_json(run)
        recorded_bytes = path.read_bytes()
        if recorded_bytes != expected_bytes:
            raise VerificationError(
                f"recorded input {input_id} differs from the current generator "
                f"(recorded sha256={sha256(recorded_bytes)}, current sha256={sha256(expected_bytes)})")
...
def write_plan(output: Path) -> tuple[list[dict[str, Any]], dict[str, dict[str, Any]]]:
    ...
    (output / "experiment-plan.csv").write_bytes(plan_bytes(experiments))
...
def execute(output: Path, experiments: list[dict[str, Any]], verify: bool = False) -> None:
    ...
    recorded_logs: dict[str, dict[str, Any]] = {}
    recorded_rows: bytes | None = None
    recorded_aggregate: str | None = None
    if verify:
        log_items = [json.loads(line) for line in
                     (output / "execution-log.jsonl").read_text(encoding="utf-8").splitlines()]
        if [item["experiment_id"] for item in log_items] != [e["experiment_id"] for e in experiments]:
            raise VerificationError("execution log experiment order or IDs differ from the current plan")
        recorded_logs = {item["experiment_id"]: item for item in log_items}
        if len(recorded_logs) != len(log_items):
            raise VerificationError("execution log contains duplicate experiment IDs")
        for exp in experiments:
            ...
            saved = recorded_logs[exp["experiment_id"]]
            if any(saved.get(key) != value for key, value in expected.items()):
                raise VerificationError(
                    f"execution log definition mismatch for {exp['experiment_id']}")
            for stream, suffix, hash_key in (("stdout", ".json", "stdout_sha256"),
                                              ("stderr", ".txt", "stderr_sha256")):
                saved_path = output / stream / f"{exp['experiment_id']}{suffix}"
                if sha256(saved_path.read_bytes()) != saved.get(hash_key):
                    raise VerificationError(
                        f"saved {stream} hash mismatch for {exp['experiment_id']}")
    ...
    for exp in experiments:
        ...
        if verify:
            ...
            if saved != log:
                raise VerificationError(f"verification mismatch in execution log for {exp['experiment_id']}")
            if sha256((stdout_dir / f"{exp['experiment_id']}.json").read_bytes()) != stdout_hash:
                raise VerificationError(f"saved stdout hash mismatch for {exp['experiment_id']}")
            if sha256((stderr_dir / f"{exp['experiment_id']}.txt").read_bytes()) != stderr_hash:
                raise VerificationError(f"saved stderr hash mismatch for {exp['experiment_id']}")
        else:
    ...
    if verify:
        if rows_bytes != recorded_rows or computed_aggregate != recorded_aggregate:
            raise VerificationError("normalized results or aggregate hash did not reproduce")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        return 1
    print("verified 108 experiments"); return 0
