...
def _artifact_prefix(mode: str, capture_mode: str, instrumentation: str) -> str:
    return f"{mode}-{capture_mode}-{instrumentation}"
...
def _parity_fail(*, scenario: str, capture_mode: str, instrumentation: str, artifact_path: str, field: str, expected: object, actual: object) -> None:
    raise SystemExit(
        f"parity check failed scenario={scenario} capture_mode={capture_mode} instrumentation={instrumentation} artifact={artifact_path} "
        f"field={field} expected={expected!r} actual={actual!r}"
    )


def _require_equal(*, scenario: str, capture_mode: str, instrumentation: str, artifact_path: str, field: str, expected: object, actual: object) -> None:
    if expected != actual:
        _parity_fail(
            scenario=scenario,
    ...
    for capture_mode in ("light", "investigation"):
        for mode in ("before", "after"):
            ...
            for instrumentation in ("native", "tracing"):
                prefix = _artifact_prefix(mode, capture_mode, instrumentation)
                run_and_analyze(
                    demo_manifest,
                    cli_manifest,
                    artifact_dir / f"{prefix}-run.json",
                    artifact_dir / f"{prefix}-analysis.json",
                    mode_arg,
                    profile=profile,
                    extra_demo_args=["--instrumentation", instrumentation, "--mode", capture_mode],
                )

        expected_files = [f"{mode}-{capture_mode}-{instrumentation}-{kind}.json" for mode in ("before", "after") for instrumentation in ("native", "tracing") for kind in ("run", "analysis")]
        missing = [name for name in expected_files if not (artifact_dir / name).exists()]
        if missing:
            raise SystemExit(f"missing parity artifacts scenario={scenario} capture_mode={capture_mode}: {', '.join(missing)}")

        before_native_run = _load_run(artifact_dir / f"before-{capture_mode}-native-run.json")
        before_tracing_run = _load_run(artifact_dir / f"before-{capture_mode}-tracing-run.json")
        after_native_run = _load_run(artifact_dir / f"after-{capture_mode}-native-run.json")
        after_tracing_run = _load_run(artifact_dir / f"after-{capture_mode}-tracing-run.json")

        before_native = load_report_json(artifact_dir / f"before-{capture_mode}-native-analysis.json")
        before_tracing = load_report_json(artifact_dir / f"before-{capture_mode}-tracing-analysis.json")
        after_native = load_report_json(artifact_dir / f"after-{capture_mode}-native-analysis.json")
        after_tracing = load_report_json(artifact_dir / f"after-{capture_mode}-tracing-analysis.json")
        ...
        for label, report in (
            ...
            if report["request_count"] <= 0:
                raise SystemExit(f"expected non-zero request count in {label} capture_mode={capture_mode} scenario={scenario}")
            if report["p95_latency_us"] <= 0:
                raise SystemExit(f"expected non-zero p95 latency in {label} capture_mode={capture_mode} scenario={scenario}")
        ...
        for label, run in (
            ...
            if len(run.get("requests", [])) == 0:
                raise SystemExit(f"expected non-zero requests in {label} run artifact capture_mode={capture_mode} scenario={scenario}")
            if scenario != "executor" and len(run.get("stages", [])) == 0:
                raise SystemExit(f"expected non-zero stages in {label} run artifact capture_mode={capture_mode} scenario={scenario}")
            routes = {r.get("route") for r in run.get("requests", [])}
            if config["route"] not in routes:
                raise SystemExit(f"expected route {config['route']} in {label} run artifact capture_mode={capture_mode} scenario={scenario}")
        ...
        if config["queues"]:
            for label, run in (("before-native", before_native_run), ("before-tracing", before_tracing_run), ("after-native", after_native_run), ("after-tracing", after_tracing_run)):
                if len(run.get("queues", [])) == 0:
                    raise SystemExit(f"expected non-zero queues in {label} run artifact capture_mode={capture_mode} scenario={scenario}")
            for run_name, run in (("before-tracing-run.json", before_tracing_run), ("after-tracing-run.json", after_tracing_run)):
                ...
                if not config["queues"].issubset(queue_names):
                    raise SystemExit(f"expected queue tracing artifact {run_name} to include queues {sorted(config['queues'])} capture_mode={capture_mode} scenario={scenario}")
                if not any(q.get("depth_at_start") is not None for q in run.get("queues", [])):
                    raise SystemExit(f"expected queue tracing queue events in {run_name} to include non-null depth_at_start capture_mode={capture_mode} scenario={scenario}")

        for run_name, run in (("before-tracing-run.json", before_tracing_run), ("after-tracing-run.json", after_tracing_run)):
            tracing_stage_names = {s.get("stage") for s in run.get("stages", [])}
            for stage in config["stages"]:
                if stage not in tracing_stage_names:
                    raise SystemExit(f"expected tracing run {run_name} to include stage '{stage}' capture_mode={capture_mode} scenario={scenario}")
            if scenario == "retry-storm" and not any(name and name.startswith("downstream_attempt_") for name in tracing_stage_names):
                raise SystemExit(f"expected tracing run {run_name} to include at least one downstream_attempt_* stage capture_mode={capture_mode} scenario={scenario}")
            if scenario in RUNTIME_SENSITIVE_TRACING_SCENARIOS:
                if not run.get("runtime_snapshots"):
                    raise SystemExit(f"expected runtime snapshots in tracing run {run_name} capture_mode={capture_mode} scenario={scenario}")
                metadata = run.get("metadata", {})
                sampler_cfg = metadata.get("effective_tokio_sampler_config")
                lifecycle_warnings = metadata.get("lifecycle_warnings") or []
                manual_disabled = any(warning.startswith("tailtriage-tracing session ran with background runtime sampling disabled") for warning in lifecycle_warnings)
                if sampler_cfg is None and not manual_disabled:
                    raise SystemExit(f"expected effective_tokio_sampler_config or disabled-sampler warning in tracing run {run_name} capture_mode={capture_mode} scenario={scenario}")
            if scenario in NON_RUNTIME_TRACING_SCENARIOS:
                _require_equal(scenario=scenario, capture_mode=capture_mode, instrumentation="tracing", artifact_path=run_name, field="runtime_snapshots", expected=[], actual=run.get("runtime_snapshots") or [])
                _require_equal(scenario=scenario, capture_mode=capture_mode, instrumentation="tracing", artifact_path=run_name, field="metadata.effective_tokio_sampler_config", expected=None, actual=(run.get("metadata") or {}).get("effective_tokio_sampler_config"))
            if scenario == "blocking" and not any(s.get("blocking_queue_depth") is not None for s in run.get("runtime_snapshots", [])):
                raise SystemExit(f"expected blocking_queue_depth runtime evidence in {run_name} capture_mode={capture_mode} scenario={scenario}")
            if scenario == "executor" and not any((s.get("global_queue_depth") is not None) or (s.get("local_queue_depth") is not None) for s in run.get("runtime_snapshots", [])):
                raise SystemExit(f"expected global/local queue runtime evidence in {run_name} capture_mode={capture_mode} scenario={scenario}")
        ...
        for label, run in (("before-native", before_native_run), ("after-native", after_native_run)):
            if "inflight" in run and len(run.get("inflight") or []) == 0:
                raise SystemExit(f"expected native inflight snapshots in {label}; tracing inflight is out of scope for prompt 3 capture_mode={capture_mode} scenario={scenario}")

        if not has_suspect_kind(before_native, {expected_kind}):
            raise SystemExit(f"expected baseline native primary suspect {expected_kind}, got {before_native['primary_suspect']['kind']} capture_mode={capture_mode} scenario={scenario}")
        if not has_suspect_kind(before_tracing, {expected_kind}):
            raise SystemExit(f"expected baseline tracing primary suspect {expected_kind}, got {before_tracing['primary_suspect']['kind']} capture_mode={capture_mode} scenario={scenario}")
        if config["require_p95_improvement"] and after_tracing["p95_latency_us"] > before_tracing["p95_latency_us"]:
            raise SystemExit(f"expected tracing mitigated p95 to be non-worse than tracing baseline, got {before_tracing['p95_latency_us']}us -> {after_tracing['p95_latency_us']}us capture_mode={capture_mode} scenario={scenario}")

        for mode, native_run, tracing_run in (("before", before_native_run, before_tracing_run), ("after", after_native_run, after_tracing_run)):
            _require_equal(scenario=scenario, capture_mode=capture_mode, instrumentation="native/tracing", artifact_path=f"{mode}-{capture_mode}-run", field="scenario_label", expected=native_run.get("scenario_label"), actual=tracing_run.get("scenario_label"))
            _require_equal(scenario=scenario, capture_mode=capture_mode, instrumentation="native/tracing", artifact_path=f"{mode}-{capture_mode}-run", field="metadata.mode", expected=(native_run.get("metadata") or {}).get("mode"), actual=(tracing_run.get("metadata") or {}).get("mode"))
            _require_equal(scenario=scenario, capture_mode=capture_mode, instrumentation="native/tracing", artifact_path=f"{mode}-{capture_mode}-run", field="metadata.effective_core_config.capture_limits", expected=_capture_limits(native_run), actual=_capture_limits(tracing_run))
            _require_equal(scenario=scenario, capture_mode=capture_mode, instrumentation="native/tracing", artifact_path=f"{mode}-{capture_mode}-run", field="route_coverage", expected=sorted({r.get("route") for r in native_run.get("requests", [])}), actual=sorted({r.get("route") for r in tracing_run.get("requests", [])}))

        print(f"tracing parity validation passed for {scenario} capture_mode={capture_mode}: baseline kind={expected_kind}, tracing p95 {before_tracing['p95_latency_us']}us -> {after_tracing['p95_latency_us']}us")
...
def validate_tracing_retention_parity(root_dir: Path, *, profile: str = "dev") -> None:
    ...
    print("tracing retention parity validation passed (tiny limits, light+investigation)")
