...
Offline import expects completed tailtriage `tt.*` tracing span JSONL (not arbitrary tracing log JSON), requires explicit span timing, and writes Run JSON before a separate `tailtriage analyze` step.
