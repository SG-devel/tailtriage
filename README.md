...
- Offline JSONL import:
  ...
  ```
- Live session path: install with `cargo add tailtriage-tracing --features live`, then add `tailtriage_tracing::TracingIntakeSession` and `session.layer()` beside your existing subscriber setup.
