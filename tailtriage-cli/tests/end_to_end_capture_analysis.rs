...
async fn queue_heavy_capture_and_analysis_reports_queue_suspect() {
    ...
    for _ in 0..6 {
        let request = tailtriage.request("/checkout");
        request
...
async fn downstream_heavy_capture_and_analysis_reports_stage_suspect() {
    ...
    for _ in 0..5 {
        let request = tailtriage.request("/checkout").with_kind("place_order");
        request
            .stage("downstream_db")
            .await_on(async {
...
#[tokio::test(flavor = "current_thread")]
async fn request_only_capture_and_analysis_reports_insufficient_evidence() {
