...
#[test]
fn import_tracing_json_default_wrapper_only_accepts_fixture() {
    let dir = tempfile::tempdir().expect("tempdir should build");
    let spans_path = dir.path().join("spans.jsonl");
    let run_path = dir.path().join("run.json");
    std::fs::write(&spans_path, complete_span_jsonl_fixture()).expect("fixture should write");
...
fn import_tracing_json_compatible_rejects_fmt_json_with_guidance() {
    ...
    assert!(stderr.contains("--input-format compatible"));
    assert!(!run_path.exists());
}
...
#[test]
fn top_level_help_includes_import_and_analyze() {
    let output = Command::new(env!("CARGO_BIN_EXE_tailtriage"))
        .arg("--help")
        .output()
        .expect("cli should run");
...
fn import_tracing_json_rejects_auto_input_format_value() {
    ...
    assert!(!output.status.success());
    let stderr = String::from_utf8_lossy(&output.stderr);
    assert!(stderr.contains("invalid value 'auto'"));
}
