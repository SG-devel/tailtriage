...
#[test]
fn import_tracing_json_fails_when_output_parent_is_a_file() {
    let dir = tempfile::tempdir().expect("tempdir should build");
    let spans_path = dir.path().join("spans.jsonl");
    let parent_file = dir.path().join("not-a-dir");
    let run_path = parent_file.join("run.json");
    std::fs::write(&spans_path, complete_span_jsonl_fixture()).expect("fixture should write");
    std::fs::write(&parent_file, "file").expect("parent placeholder file should write");
    ...
    assert!(
        stderr.contains("failed to create output parent directory")
            || stderr.contains(&parent_file.display().to_string())
    );
