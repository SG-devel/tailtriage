...
enum Command {
    ...
    Analyze {
        ...
        /// Analyzer option override in PATH=VALUE form. Can be repeated.
        #[arg(long, value_name = "PATH=VALUE")]
        analyzer_set: Vec<String>,
        /// Print analyzer option help and exit.
        #[arg(long)]
        ...
        Command::Analyze {
            ...
            let run_json = run_json.ok_or_else(|| {
                clap::Error::raw(
                    ...
                    "missing required RUN_JSON argument (use --help-analyzer-options to print analyzer options without loading an artifact)",
                )
            })?;
...
fn build_analyze_options(
    ...
    if let Some(path) = analyzer_config {
        let toml = std::fs::read_to_string(path)
            .map_err(|e| format!("failed to read analyzer config {}: {e}", path.display()))?;
        options = options.merge_toml_str(&toml)?;
    }

    options.apply_overrides(analyzer_set)?;
    Ok(options)
}

fn print_analyzer_options_help() {
    for descriptor in analyze_option_descriptors() {
        ...
        if let Some(note) = descriptor.increasing {
            println!("  increasing: {note}");
        }
        if let Some(note) = descriptor.decreasing {
            println!("  decreasing: {note}");
        }
        println!();
    }
}
