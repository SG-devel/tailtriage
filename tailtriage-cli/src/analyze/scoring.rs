...
pub(crate) fn clamp_score(value: u64) -> u8 {
    u8::try_from(value.min(100)).unwrap_or(100)
}

pub(crate) fn nonzero_sample_count(values: &[u64]) -> usize {
    values.iter().filter(|&&v| v > 0).count()
}

pub(crate) fn max_or_zero(values: &[u64]) -> u64 {
    values.iter().copied().max().unwrap_or(0)
}

pub(crate) fn score_sample_quality(sample_count: usize) -> u8 {
...
pub(crate) fn score_from_permille(base: u64, permille: u64, scale: u64) -> u64 {
    base + permille.min(1000) / scale
}

pub(crate) fn cap_unless_clean_evidence(score: u64, clean: bool, soft_cap: u8) -> u8 {
    if clean {
        clamp_score(score)
    } else {
...
pub(crate) fn queue_saturation_suspect(
    run: &Run,
...
pub(crate) fn blocking_signal(run: &Run) -> Option<BlockingSignal> {
...
pub(crate) fn strong_blocking_signal(signal: BlockingSignal) -> bool {
    signal.p95 >= 12 && signal.peak >= 20 && signal.nz_share_permille >= 700 && signal.samples >= 30
}

pub(crate) fn stage_correlates_with_blocking_pool(stage: &str) -> bool {
...
pub(crate) fn blocking_pressure_suspect(run: &Run) -> Option<Suspect> {
...
pub(crate) fn executor_pressure_suspect(
    run: &Run,
    inflight_trend: Option<&InflightTrend>,
) -> Option<Suspect> {
...
pub(crate) fn downstream_stage_candidates(
    run: &Run,
    p95_req: u64,
    total_req: u64,
) -> Vec<StageCandidate> {
...
pub(crate) fn downstream_stage_suspect(run: &Run) -> Option<Suspect> {
