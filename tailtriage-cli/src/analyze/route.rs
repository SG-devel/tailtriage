...
pub(crate) struct RouteBreakdownContext {
    pub(crate) breakdowns: Vec<RouteBreakdown>,
    pub(crate) divergent: bool,
}

pub(crate) fn route_breakdowns(run: &Run, global: &Report) -> RouteBreakdownContext {
...
pub(crate) fn route_divergence(candidates: &[RouteBreakdown]) -> bool {
    candidates
        .iter()
...
pub(crate) fn should_emit_route_breakdowns(global: &Report, candidates: &[RouteBreakdown]) -> bool {
    if candidates.len() < 2 {
        return false;
    }
...
pub(crate) fn filtered_run_for_route(run: &Run, request_ids: &[String]) -> Run {
