...
pub(crate) fn evidence_quality(run: &Run) -> EvidenceQuality {
...
pub(crate) fn request_status(run: &Run) -> SignalCoverageStatus {
...
pub(crate) fn family_status(is_empty: bool, dropped: u64) -> SignalCoverageStatus {
    if dropped > 0 {
        SignalCoverageStatus::Truncated
    } else if is_empty {
        SignalCoverageStatus::Missing
...
pub(crate) fn runtime_status(run: &Run) -> SignalCoverageStatus {
...
pub(crate) fn evidence_limitations(
    run: &Run,
...
pub(crate) fn truncation_warnings(run: &Run) -> Vec<String> {
