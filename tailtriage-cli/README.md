...
`tailtriage import tracing-spans-jsonl` imports **completed `tt.*` tailtriage tracing span JSONL** into **Run JSON** (not Report JSON).
...
- `compatible` is only for pre-stable/internal normalized completed-span shapes with explicit start/end timestamps; it is not auto-detection and not generic tracing log JSON import.
...
The command imports completed `tt.*` tailtriage tracing span JSONL records in the documented shape and writes Run JSON through the normal local JSON artifact writer, not Report JSON. Import warnings are printed to stderr as `warning: ...`. Analysis is a separate step: `tailtriage analyze tailtriage-run.json`.
