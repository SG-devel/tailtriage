...
Import fails when zero request events would be written (for example unrelated-only input or all-skipped malformed `tt.*` input). Persisted CLI artifacts (Run JSON and completed-span JSONL export) require at least one completed request because `tailtriage analyze` requires at least one request in CLI-loaded run artifacts.
