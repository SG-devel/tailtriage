...
TracingSession validation paths may run deterministic/manual runtime snapshot mode (`disable_background_sampler()` + `record_runtime_snapshot(...)`) to remove sampler cadence noise. This supports repeatable triage evidence and does not claim causal proof.
