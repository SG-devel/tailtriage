...
Broad tracing parity checks scenario coverage, expected evidence presence, route coverage, mode parity, and runtime/non-runtime semantics via `scripts/demo_tool.py validate-tracing-parity all --profile release`. The queue tiny-limit retention parity gate checks retained counts, dropped counters, `truncation.limits_hit`, and `metadata.effective_core_config` via `scripts/demo_tool.py validate-tracing-retention-parity --profile release`. Runtime-sensitive tracing scenarios (Tokio session coupling) must include runtime snapshots plus sampler metadata; tracing-only scenarios must not fabricate runtime snapshots. These checks support triage consistency and do not prove universal production performance or root-cause certainty.
