...
Native/tracing parity is CI-gated via `scripts/demo_tool.py validate-tracing-parity all --profile release`, and each scenario runs in both `light` and `investigation` capture modes. Broad tracing parity checks scenario coverage, expected evidence presence, route coverage, mode parity, and runtime/non-runtime semantics. The queue tiny-limit retention parity gate (`scripts/demo_tool.py validate-tracing-retention-parity --profile release`) also runs in `light` and `investigation` and checks retained counts, dropped counters, `truncation.limits_hit`, and `metadata.effective_core_config`. Runtime-sensitive tracing scenarios (Tokio session coupling) must include deterministic manually injected runtime snapshots plus either effective Tokio sampler metadata (normal background-sampler sessions) or an explicit disabled-background-sampler lifecycle warning (deterministic/manual runtime snapshot mode). Tracing-only scenarios must not fabricate runtime snapshots. Parity here means request/stage/queue/runtime-evidence shape and diagnosis direction, not exact latency equality, universal production performance proof, or root-cause certainty.
