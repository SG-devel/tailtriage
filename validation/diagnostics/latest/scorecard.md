...
The diagnostic manifest and benchmark own current analyzer-execution, accuracy-observation, and report-contract accounting. Report-contract pass rates do not contribute to analyzer accuracy; equivalent artifact encodings are one logical observation, not independent samples. Fixture labels are controlled intent, not production truth. These checks are not real-service validation and do not provide root-cause proof.
