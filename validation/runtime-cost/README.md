...
CI additionally runs one bounded runtime-cost smoke gate on Ubuntu extended release using one warmup round plus the measured-round count shown in `.github/workflows/ci.yml`, then runs `scripts/validate_runtime_cost_summary.py`. Runtime-cost CI is a bounded smoke gate with broad tracing/native sanity thresholds. It is not a rigorous benchmark suite and should not be interpreted as stable performance characterization. Outputs are validated in-place and are not uploaded by default. Full runtime-cost measurement remains local/developer-run and machine/workload/profile scoped.
